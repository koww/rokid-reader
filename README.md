**这个项目是对[rokid-reader](https://github.com/qhduan/rokid-reader)的Fork，针对使用[R08蓝牙指环](https://github.com/Anezium/R08-Access-Bridge)进行了部分改动**
---
# Rokid 智能眼镜小说阅读器
...
| BACK | 退出程序 | 回书架 |
| 指环单击 | 打开书 / 切换传书 | 下一页 |
| 指环前滑 / 后滑 | 下移 / 上移选中 | 下一页 / 上一页 |
| 指环双击 | 退出程序 | 回书架 |

指环走无障碍动作通道（需要 R08 Access Bridge 这类无障碍服务已启用）：阅读页本身不响应普通触摸，所以指环的单击/滑动是以无障碍点击和滚动的形式生效的。
...
- **无障碍动作通道**：阅读页/书架在无障碍节点上上报 click + scroll 动作，蓝牙指环无需触摸即可翻页；只改节点信息不改 View 触摸行为，避免滚轮按压翻两页
